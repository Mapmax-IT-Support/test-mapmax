...
import {
  renderMarker,
  BLUE_MARKER,
} from "../../Map/GoogleMaps/GoogleMapComponents";
...
class ListingView extends React.Component {
    ...
    if (this.props.urlParams) {
      if (this.props.urlParams.listingId != undefined) {
        ...
        if (listing.length > 0) {
          ...
          if (
            !listing[0].published &&
            listing[0].user_id !== this.props.user._id
          ) {
  ...
  getUrl = () => {
    this.props.history.push(
      '/"' + encodeURI(this.state.listingData.location.formatted) + '"/none'
    );
  };
    ...
    return (
      <div className="listingPage">
        <div
          ...
          style={this.state.listingData == undefined ? {} : { display: "none" }}
        >
        ...
        {this.state.listingData != undefined && (
          <div className="listingContainer">
            <div className="listingHeader">
              <div style={{ display: "flex", flex: 1 }}>
                <h3 className="listingAddressText">
                  {this.state.listingData.location.formatted}
                </h3>
              </div>
              <div
                style={{
            ...
            <MediaQuery minDeviceWidth={551}>
              <div className="listingMediaContainer">
                <div
                  ...
                  style={{ height: "400px", width: "39.25%" }}
                >
                  <GoogleMapReact
                    ...
                    onGoogleApiLoaded={({ map, maps }) =>
                      this.apiIsLoaded(
                        map,
                        maps,
                        this.state.listingData.location.coords
                      )
                    }
                ...
                <Slideshow
                  width={"60%"}
                  height={"400px"}
                  photos={this.state.listingData.photos?.cover_photos.concat(
                    this.state.listingData.photos.site_photos
                  )}
                />
              </div>
            </MediaQuery>
            ...
            <div className="listingViewBottom">
              <div style={{ width: "65%", margin: "2.5%" }}>
                <h3 className="listingTableHeader" style={{ color: "white" }}>
                  Listing Summary
                </h3>
                <Table
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className="tableValue">
                        {data.pricingInfo.askingPrice != undefined
                          ? `$ ${data.pricingInfo.askingPrice}`
                          : "Unavailable"}
                      </td>
                    </tr>
                    <tr>
                      <td>For Sale?</td>
                      <td className="tableValue">
                        {capitalizeFirst(data.contactInfo.forSale)}
                      </td>
                    </tr>
                    <tr>
                      <td>For Lease?</td>
                      <td className="tableValue">
                        {capitalizeFirst(data.contactInfo.forLease)}
                      </td>
                    </tr>
                  </tbody>
                </Table>
                ...
                {Object.entries(data.locationDetails).length == 0 && (
                  <p style={{ color: "white", textAlign: "left" }}>
                    This information is unavailable or has not been provided yet
                  </p>
                )}
                <Table
                  className="listingDataTable"
                ...
                >
                  <tbody>
                    {Object.entries(data.locationDetails).map(
                      ([key, value], i) => {
                        // C/O
                ...
                {Object.entries(data.pricingInfo).length == 0 && (
                  <p style={{ color: "white", textAlign: "left" }}>
                    This information is unavailable or has not been provided yet
                  </p>
                )}
              ...
              <div className="contact_info_panel">
                ...
                {data.photos?.contact_photos.length > 0 && (
                  <Image
                    src={S3_BASE + data.photos?.contact_photos[0]}
                ...
                >
                  {data.contactInfo.relationship != "other"
                    ? capitalizeFirst(data.contactInfo.relationship)
                    : "Other (non broker/landlord)"}
                </p>
                <div style={{ padding: "0.5em" }}>
                  <p style={{ color: "white" }}>
                    {data.contactInfo.ownershipDetails}
                  </p>
                </div>
              </div>
            </div>
