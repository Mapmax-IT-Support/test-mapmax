  ...
  render() {
    ...
    if (this.props.places.length > 0) {
      places = this.props.places.map((place, i) => {
        ...
        return (
          <tr key={i} onClick={() => this.onUpdateActivePlace(place)}>
            <td>{i + 1}</td>
            <td>{place.name}</td>
            <td>
              {<img src={icon} style={{ width: "32px", height: "32px" }}></img>}
            </td>
          </tr>
        );
      });
    ...
    return (
      <div className="places-list">
        <div id="places-list_header">
          <br></br>
          <p style={{ textAlign: "center", color: textPrimary }}>
            <strong>Competitors and/or Generators</strong>
          </p>
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <p style={{ color: textPrimary }}>Businesses / Points of Interets</p>
            {/* <img style={{ width: 32, height: 32 }} src={YELLOW_MARKER}></img>
            <p style={{ color: textPrimary }}>Interests</p> */}
          </div>
        </div>
